...
func (c *LinkerMsgCreator) Create(session ares.ISession, envelope *pb.Envelope) (ares.IMsg, error) {
	...
	if ok {
		...
		if envelope.GetPvId() != 0 {
			...
			toSession, ok := provider.Sessions().GetSession(envelope.GetPvId()).(*ProviderSession)
			if toSession == nil || !ok {
