...
func NewMsgCreator(linker *Linker) *MsgCreator {
...
func (c *MsgCreator) Create(session ares.ISession, envelope *pb.Envelope) (ares.IMsg, error) {
	...
	if ok {
		...
		if envelope.GetPvId() != 0 {
			...
			toSession, ok := c.linker.provider.GetSessions().GetSession(envelope.GetPvId()).(*provider.Session)
			if toSession == nil || !ok {
			...
			if toSession.CheckToProvide() && !linkerSession.CheckState(int(pb.ClientState_TOPROVIDEE)) {
				...
				c.linker.CloseSession(linkerSession, pb.SessionError_CANT_DISPATCH)
				return nil, nil
			}
			// 白名单
			if linkerSession.WhiteFilterByProvider(toSession) {
				...
				c.linker.CloseSession(linkerSession, pb.SessionError_OPEN_WHITE_IP)
				return nil, nil
			}
			// 黑名单
			if linkerSession.BlackFilterByProvider(toSession) {
				...
				c.linker.CloseSession(linkerSession, pb.SessionError_OPEN_BLACK_IP)
				return nil, nil
			}
